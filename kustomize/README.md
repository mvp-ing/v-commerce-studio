...
Optionally, [install the `kustomize` binary](https://kubectl.docs.kubernetes.io/installation/) to avoid manually editing a `kustomization.yaml` file. v-commerce's instructions will often use `kustomize edit` (like `kustomize edit add component components/some-component`), but you can skip these commands and instead add components manually to the [`/kustomize/kustomization.yaml` file](/kustomize/kustomization.yaml).

You need to have a Kubernetes cluster where you will deploy the v-commerce's Kubernetes manifests. To set up a local cluster, you can follow the instructions in the [Development Guide](/docs/development-guide.md).

## Deploy v-commerce with Kustomize
...
1. Access the web frontend using port-forwarding:
...
## Deploy v-commerce variations with Kustomize
...
- [**Change to the Cymbal Shops Branding**](components/cymbal-branding)
  - Changes all v-commerce-related branding to Cymbal Shops. The code adds/enables an environment variable `CYMBAL_BRANDING` in the `frontend` service.
- [**Integrate with Observability Tools**](components/google-cloud-operations)
  - Enables Monitoring (Stats), Tracing, and Profiler for various services within v-commerce.
- [**Integrate with External Redis**](components/memorystore)
  - The default v-commerce deployment uses the in-cluster `redis` database for storing the contents of its shopping cart. This variation overrides the default database with an external Redis instance.
- [**Integrate with Spanner**](components/spanner)
  - The default v-commerce deployment uses the in-cluster `redis` database for storing the contents of its shopping cart. The Spanner deployment variation overrides the default database with a Spanner database.
- [**Integrate with AlloyDB**](components/alloydb)
  - The default v-commerce deployment uses the in-cluster `redis` database for storing the contents of its shopping cart. The AlloyDB deployment variation overrides the default database with an AlloyDB database.
- [**Secure with Network Policies**](components/network-policies)
  - Deploy fine granular `NetworkPolicies` for v-commerce.
...
To customize v-commerce with its variations, you need to update the default `kustomize/kustomization.yaml` file. You could do that manually, use `sed`, or use the `kustomize edit` command like illustrated below.
...
You could now combine it with other variations, like for example with the observability variation:
...
kind: Kustomization
resources:
  - <your-repo-url>/kustomize/base
components:
  - <your-repo-url>/kustomize/components/cymbal-branding
  - <your-repo-url>/kustomize/components/google-cloud-operations
```
