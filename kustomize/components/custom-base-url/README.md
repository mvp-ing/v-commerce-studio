# Customize the Base URL for v-commerce

This component allows you to change the base URL for the v-commerce application. By default, the application uses the root path ("/") as its base URL. This customization sets the base URL to "/v-commerce" and updates the health check paths accordingly.

## What it does

1. Sets the `BASE_URL` environment variable to "/v-commerce" for the frontend deployment.
2. Updates the liveness probe path to "/v-commerce/_healthz".
3. Updates the readiness probe path to "/v-commerce/_healthz".
