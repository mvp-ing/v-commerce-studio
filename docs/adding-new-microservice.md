...
This document outlines the steps required to add a new microservice to the v-commerce application.
...
Refer to the existing Dockerfiles in the `src/` directory for examples.
...
Refer to the existing components in `/kustomize/components/` for examples.
